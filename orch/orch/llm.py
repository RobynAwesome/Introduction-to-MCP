...
def call_ai(agent: Agent, prompt: str, temperature: float = 0.7) -> str:
    """Unified entry point for Gemini 3.1 High-Thinking and Structured fallback."""
    ...
    if agent.provider.lower() == "gemini":
        try:
            ...
            # Fallback to standard 3.1 if the thinking preview is not active
            print(f"High-Thinking Model failed: {str(e)}. Falling back to standard Gemini 3.1.")
            return call_ai_litellm(agent, prompt, temperature)
            
    return call_ai_litellm(agent, prompt, temperature)

def call_ai_litellm(agent: Agent, prompt: str, temperature: float = 0.7) -> str:
    """Standard LiteLLM logic with Gemini 3 migration mapping."""
    model_map = {
        # Migrated identifiers (as of March 2026)
...
def call_gemini_thinking(agent: Agent, prompt: str) -> str:
    """Uses the modern google-genai SDK for Gemini 3.1 High-Thinking."""
    client = genai.Client(api_key=agent.api_key)
    
    # Master Blueprint: Gemini 3.1 Pro Preview with HIGH thinking
    ...
    for chunk in client.models.generate_content_stream(
        ...
        if chunk.parts and chunk.parts[0].text:
            full_text.append(chunk.parts[0].text)
